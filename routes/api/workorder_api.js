...
router.get('/', function(req, res, next) {
	conn
		...
		.catch(function(err) {
			console.log(err);
			conn.close();
			res.send('Error while fetching Work Orders data');
		});
});
...
router.get('/:id', function(req, res, next) {
	conn
		...
		.then(function() {
			...
			var sqlQuery = 'SELECT * FROM [SPM_Database].[dbo].[WorkOrderManagement] where WorkOrder=@id';
			request
				.query(sqlQuery)
				...
				.catch(function(err) {
					console.log(err);
					conn.close();
					res.status(400).send('Error while fetching WorkOrder data with id: ' + req.params.id);
				});
		})
		.catch(function(err) {
			console.log(err);
			conn.close();
			res.send('Error while fetching WorkOrder data with id: ' + req.params.id);
		});
});
